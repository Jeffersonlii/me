...
function Skills() {
  //TODO:refactor this with list elements
  const skillsInnertext = [];
  for (const [skillGroup, skills] of Object.entries(
    json.qualifications['Technical Skills']
  )) {
    const innerskills = [];
  ...
  const hobbies = [];
  for (const [hobby, desc] of Object.entries(
    json.qualifications['Interests and Hobbies']
  )) {
    const inner = `
                <br>
                <b>${hobby}</b>
    ...
    hobbies.push(<div dangerouslySetInnerHTML={{ __html: inner }} />);
  }

  return (
    <>
      <div id="quals-wrap">
        <div id="skills-wrap">{skillsInnertext}</div>
