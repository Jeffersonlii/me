...
export default function OSHeader(props: any){
    ...
    let dropdowns: DropDownLabel[] = dropDownStatics.map((el, index)=>{
        return {...el, id: index, options: 
            el.options.map((op, index)=>({...op, id: index, disabled: op.disabled ?? false})
        )}
    })

    const [group, setGroup] = useState(dropdowns.map((a)=>({...a, opened: false})))
    ...
    return (
        ...
        {group.map((label) => {
        return <div className="link" key={label.id} 
                onMouseOver={(e)=>onToggle(label.id, "enter")}
                onMouseLeave={(e)=>onToggle(label.id, "leave")} >
                    {label.label}
                    {(()=>{
                        if(label.opened) 
                            return <DropPanel label={label} ></DropPanel>
                    })()}
                </div>})}
        </>)
}
...
function DropPanel(props: { label: DropDownLabel}){  
    return <div className="panel">
                {props.label.options.map((op) => { 
                   ...
                   return (op.disabled ?
                     <div className="option disabled" key={op.id}>{op.option}</div>
                     :
                     <div className="option" key={op.id}>{op.option}</div>)
                })}
            </div>
}
